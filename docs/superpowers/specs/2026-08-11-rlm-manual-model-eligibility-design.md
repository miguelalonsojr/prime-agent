...
Manual model selection reads the current `ModelRegistry.getAvailable()` catalog. RLM uses `ModelRegistry.getExecutableModels()`, which applies an additional `openai-codex` account-catalog request and exact model-ID intersection. The two paths can disagree: a configured Codex model can be selected manually but omitted from RLM discovery and rejected for an explicit child.
...
`AgentSession._authenticatedRlmModels()` will obtain its catalog from `ModelRegistry.getAvailable()`. It will retain the existing stale-auth status filter. Manual selection refreshes the registry when availability changes, so both paths read the same current catalog without resetting dynamic provider registrations during an RLM lookup.
