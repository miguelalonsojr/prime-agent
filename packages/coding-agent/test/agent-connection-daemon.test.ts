...
describe("DaemonAgentConnection", () => {
	...
	it("recovers a supervisor close that occurs during initial direct attach", async () => {
		const supervisor = new FakeDaemonClient();
		let closeSent = false;
		const direct = {
			isConnected: true,
		...
		const connection = await DaemonAgentConnection.attach(routed, "active-1");
		for (let attempt = 0; attempt < 50 && supervisor.reconnectCount === 0; attempt++) {
	...
	it("resets a half-open supervisor handshake without closing the healthy direct socket", async () => {
		...
		for (let attempt = 0; attempt < 200 && supervisor.reconnectCount < 2; attempt++) {
			...
			request: async (command: DaemonCommand) => {
				...
				if (command.type === "get_state") {
					return {
						...
						success: true as const,
						data: createConnectionState(command.activeSessionId, "source-session"),
					};
				}
